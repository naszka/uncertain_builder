...
def download(url, destination, data_prefix, description="downloading dataset into"):
    ...
    with open(destination, "wb") as f:
        with tqdm(desc=description, total=(total_length // CHUNK_SIZE) + 1) as pbar:
...
class BlobFileDownloader:
    def __init__(self, local_blob_path=None):
        try:
            ...
            raise ImportError(
                "please install azure blob storage via pip: pip install azure-storage-blob"
            )
        self.sas_token = os.getenv("IGLU_SAS_TOKEN")
        self.sas_url = (
            "https://igludatacollection.blob.core.windows.net/iglu-data-task-2?"
            + self.sas_token
        )
    ...
    def download_blobs_in_container(self, prefix):
        ...
        for blob in tqdm(to_download, desc="downloading dataset"):
            ...
            content = (
                self.container_client.get_blob_client(blob).download_blob().readall()
            )
